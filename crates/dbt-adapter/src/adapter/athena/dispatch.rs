...
use super::aws::{AthenaAws, RelationParts};
use super::{
...
impl Adapter {
    ...
    fn athena_aws(&self) -> Result<Option<Arc<AthenaAws>>, JinjaError> {
        match &self.inner {
            Typed { adapter, .. } => Ok(Some(AthenaAws::for_engine(adapter.engine().as_ref())?)),
            Parse(_) => Ok(None),
        }
    }
    ...
    pub fn athena_get_glue_table_type(&self, args: &[Value]) -> Result<Value, JinjaError> {
        let iter = ArgsIter::new("get_glue_table_type", &["relation"], args);
        let relation = relation_parts(iter.next_arg::<&Value>()?)?;
        iter.finish()?;
        let Some(aws) = self.athena_aws()? else {
    ...
    pub fn athena_clean_up_table(&self, args: &[Value]) -> Result<Value, JinjaError> {
        let iter = ArgsIter::new("clean_up_table", &["relation"], args);
        let relation = relation_parts(iter.next_arg::<&Value>()?)?;
        iter.finish()?;
        if let Some(aws) = self.athena_aws()?
    ...
    pub fn athena_delete_from_glue_catalog(&self, args: &[Value]) -> Result<Value, JinjaError> {
    ...
    pub fn athena_drop_glue_database(&self, args: &[Value]) -> Result<Value, JinjaError> {
        let iter = ArgsIter::new(
            "drop_glue_database",
    ...
    pub fn athena_expire_glue_table_versions(&self, args: &[Value]) -> Result<Value, JinjaError> {
        let iter = ArgsIter::new(
    ...
    pub fn athena_swap_table(&self, args: &[Value]) -> Result<Value, JinjaError> {
    ...
    pub fn athena_clean_up_partitions(&self, args: &[Value]) -> Result<Value, JinjaError> {
        let iter = ArgsIter::new(
            "clean_up_partitions",
    ...
    pub fn athena_delete_from_s3(&self, args: &[Value]) -> Result<Value, JinjaError> {
    ...
    pub fn athena_upload_seed_to_s3(&self, args: &[Value]) -> Result<Value, JinjaError> {
        let iter = ArgsIter::new(
            "upload_seed_to_s3",
            &[
                "relation",
